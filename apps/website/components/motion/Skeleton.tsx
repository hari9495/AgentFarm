...
export function Skeleton({ className, rounded = 'rounded-lg' }: { className?: string; rounded?: string }) {
  ...
  return (
    <div className={cn('relative overflow-hidden', rounded, className)} style={{ background: 'var(--op-paper-3)' }} aria-hidden>
      {!reduce && (
        <motion.div
...
export function SkeletonCard({ className }: { className?: string }) {
  return (
    <div className={cn('rounded-2xl p-6', className)} style={{ background: 'var(--op-paper)', border: '1px solid var(--op-line)' }}>
