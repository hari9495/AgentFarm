...
Auth: Most routes require a valid `agentfarm_session` cookie or `Authorization: Bearer <token>` header. Unauthenticated routes are marked with `â€”` in the Auth column.
...
|---|---|---|---|---|---|
| POST | `/auth/login` | â€” | `{email, password}` | `{token, userId, tenantId, workspaceIds, scope, expiresAt}` | HMAC-signed session token |
| POST | `/auth/signup` | â€” | `{email, password, name, tenantName}` | `{token, userId, tenantId}` | Creates tenant + user |
| POST | `/auth/logout` | required | â€” | `{ok: true}` | Clears session |
| GET | `/auth/session` | required | â€” | `SessionPayload` | Verify current session |
| POST | `/auth/internal-login` | â€” | `{email, password}` | `{token}` | Internal ops login (dashboard) |
...
| GET | `/v1/approvals/:approvalId` | required | â€” | `ApprovalRecord` | Get approval by ID |
...
| POST | `/v1/approvals/:approvalId/escalate` | required | â€” | `{ok: true}` | Mark as escalated |
...
| Method | Path | Auth | Request Body | Response | Notes |
|---|---|---|---|---|---|
| GET | `/v1/billing/plans` | â€” | â€” | `Plan[]` | List active plans |
...
| POST | `/v1/billing/stripe/webhook` | â€” | (Stripe event body) | `{ok: true}` | Stripe HMAC-verified webhook |
| POST | `/v1/billing/razorpay/webhook` | â€” | `{razorpay_order_id, razorpay_payment_id, razorpay_signature, orderId}` | `{ok: true}` | Razorpay signature-verified webhook |
| GET | `/v1/billing/orders/:orderId` | required | â€” | `OrderRecord` | Get order |
...
| Method | Path | Auth | Request Body | Response | Notes |
|---|---|---|---|---|---|
| POST | `/v1/webhooks/zoho-sign` | â€” (token header) | Zoho Sign event | `{ok: true}` | `x-zoho-webhook-token` header verified; marks order signed, triggers ProvisioningJob |
...
| GET | `/v1/connectors/auth/callback` | â€” | `?code=&state=` | redirect | OAuth code exchange callback |
| GET | `/v1/connectors/:connectorId/status` | required | â€” | `ConnectorStatus` | Get auth status |
| DELETE | `/v1/connectors/:connectorId/auth` | required | â€” | `{ok: true}` | Revoke connector |
...
| GET | `/v1/tasks/sse/:workspaceId` | required | â€” | SSE stream | Task progress stream |
...
| Method | Path | Auth | Request Body | Response | Notes |
|---|---|---|---|---|---|
| GET | `/v1/language/tenant` | required | â€” | `TenantLanguageConfig` | Tenant-level language config |
| PATCH | `/v1/language/tenant` | required | `{defaultLanguage?, ticketLanguage?, autoDetect?}` | Updated config | |
| GET | `/v1/language/workspace/:workspaceId` | required | â€” | `WorkspaceLanguageConfig` | |
| PATCH | `/v1/language/workspace/:workspaceId` | required | `{preferredLanguage?}` | Updated config | |
| GET | `/v1/language/user/:userId` | required | â€” | `UserLanguageProfile` | |
...
| Method | Path | Auth | Request Body | Response | Notes |
|---|---|---|---|---|---|
| GET | `/v1/mcp` | required | â€” | `TenantMcpServer[]` | List active servers |
| POST | `/v1/mcp` | required | `{name, url, workspaceId?, headers?}` | `TenantMcpServer` | Register or reactivate |
| GET | `/v1/mcp/:id` | required | â€” | `TenantMcpServer` | Get by ID |
| DELETE | `/v1/mcp/:id` | required | â€” | `{ok: true}` | Deactivate server |
...
| GET | `/v1/admin/provision/:jobId` | required (internal) | â€” | `ProvisioningJob` | Get job status |
...
| GET | `/v1/meetings/:sessionId` | required | â€” | `MeetingSession` | Get session |
| POST | `/v1/meetings/:sessionId/end` | required | â€” | `{summary, actionItems}` | End and summarize |
| DELETE | `/v1/meetings/:sessionId` | required | â€” | `{ok: true}` | Delete session |
...
| GET | `/v1/questions/:questionId` | required | â€” | `AgentQuestion` | Poll for answer |
...
| Method | Path | Auth | Request Body | Response | Notes |
|---|---|---|---|---|---|
| GET | `/v1/observability/health` | â€” | â€” | `{status: "ok"}` | Health check |
| GET | `/v1/observability/metrics` | ops token | â€” | `MetricsSummary` | Aggregated metrics |
| GET | `/v1/ops/sla-summary` | ops token | â€” | `OpsSlaSummary` | Provisioning SLA report |
...
| Method | Path | Auth | Request Body | Response | Notes |
|---|---|---|---|---|---|
| GET | `/v1/workspace-session/:workspaceId` | required | â€” | `WorkspaceSessionState` | Get session state |
...
| GET | `/v1/desktop-profile/:workspaceId` | required | â€” | `DesktopProfile` | |
| PUT | `/v1/desktop-profile/:workspaceId` | required | `{browser, tabState, ...}` | Updated profile | |
| GET | `/v1/ide-state/:workspaceId` | required | â€” | `IdeState` | |
...
| GET | `/v1/pull-requests/:draftId` | required | â€” | `PrDraft` | |
| POST | `/v1/pull-requests/:draftId/submit` | required | â€” | `{prId, url}` | Submit to provider |
| POST | `/v1/ci-failures` | required | `{workspaceId, provider, runId, repo, branch, failedJobs, logRefs}` | `{reportId}` | Create triage report |
| GET | `/v1/ci-failures/:reportId` | required | â€” | `CiTriageReport` | |
...
| Method | Path | Auth | Request Body | Response | Notes |
|---|---|---|---|---|---|
| GET | `/v1/roles` | required | â€” | `RoleDefinition[]` | All available agent roles |
| GET | `/v1/snapshots/:botId` | required | â€” | `BotCapabilitySnapshot` | Latest capability snapshot |
...
| GET | `/v1/env-reconciler/:workspaceId` | required | â€” | `EnvProfile` | Environment profile |
| POST | `/v1/env-reconciler/:workspaceId/reconcile` | required | â€” | `{driftReport}` | Trigger reconciliation |
...
| GET | `/v1/skill-scheduler` | required | â€” | `ScheduledSkill[]` | List scheduled skills |
...
| Method | Path | Auth | Request Body | Response | Notes |
|---|---|---|---|---|---|
| GET | `/v1/plugins` | required | â€” | `Plugin[]` | List loaded plugins |
...
| Method | Path | Notes |
|---|---|---|
| POST | `/api/auth/login` | Auth bridge â€” stores both gateway + internal session cookie |
