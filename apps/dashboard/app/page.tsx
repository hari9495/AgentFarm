                ...
                <div className={`mission-control ${unifiedView ? 'mission-control-all' : ''} ${compactMode ? 'mission-control-compact' : ''} ${presentationMode ? 'mission-control-present' : ''}`}>
                    ...
                    {(unifiedView || activeTab === 'overview') && (
                        <section id="dashboard-panel-overview" role="tabpanel" aria-labelledby="dashboard-tab-overview" className="dashboard-panel mission-section" style={{ '--stagger-index': '0' } as React.CSSProperties}>
                            ...
                            <CommandCenterOverview
                                stats={{ onShift: summary.active_bots, tasksLive: summary.total_workspaces, approvals: dashboardSlice.pendingApprovals.length }}
                                kpis={targetKpis.map((k) => ({ label: k.label, value: k.value, trend: k.trend, delta: k.delta, deltaTone: k.deltaTone, status: k.status, statusTone: k.statusTone }))}
                                health={{ workspaces: systemHealthPct, approvals: approvalHealthPct, connectors: connectorHealthPct }}
                                approvals={dashboardSlice.pendingApprovals}
                                summary={{ tenant_name: summary.tenant_name, plan_name: summary.plan_name, tenant_status: summary.tenant_status, pending_approvals: summary.pending_approvals, total_workspaces: summary.total_workspaces }}
                                workspace={{ workspace_name: workspace.workspace_name, role_type: workspace.role_type, workspace_status: workspace.workspace_status, bot_status: workspace.bot_status, runtime_tier: workspace.runtime_tier, latest_incident_level: workspace.latest_incident_level }}
                                estimatedCost={usageSummary.estimated_cost}
                                events={dashboardSlice.events}
                                source={source}
                            />
                        </section>
                    )}
                    ...
                    {(unifiedView || activeTab === 'marketplace') && (
                        <section id="dashboard-panel-marketplace" role="tabpanel" aria-labelledby="dashboard-tab-marketplace" className="dashboard-panel mission-section" style={{ '--stagger-index': '4' } as React.CSSProperties}>
                            ...
                            <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12, padding: '12px 16px', borderRadius: 12, background: 'rgba(45, 138, 138,0.06)', border: '1px solid rgba(45, 138, 138,0.18)', marginBottom: 16 }}>
                                ...
                                <div style={{ fontSize: 13, color: 'var(--ink-soft)', lineHeight: 1.55 }}>
                                    ...
                                    {' '}<a href="/connectors" style={{ color: 'var(--accent)', fontWeight: 700, textDecoration: 'none' }}>Go to Connectors → Install Connectors tab →</a>
                                </div>
                            </div>
