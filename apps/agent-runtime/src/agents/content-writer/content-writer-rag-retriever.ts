...
export async function buildContentWriterRagContext(
    ...
    const [similarContent, brandGuideChunks, lessons] = await Promise.all([
        config?.usePriorWork !== false ? retrieveSimilarContent(query, gatewayBaseUrl, serviceToken) : Promise.resolve([]),
        config?.useTemplates !== false ? retrieveBrandGuide(query, gatewayBaseUrl, serviceToken) : Promise.resolve([]),
        config?.useLessons   !== false ? retrieveContentLessons(query.tenantId, workspaceId, gatewayBaseUrl, serviceToken) : Promise.resolve([]),
    ]);
