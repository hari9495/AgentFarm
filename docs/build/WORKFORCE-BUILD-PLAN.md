...
| 2026-06-25 | GTM2 done | Managed MCP catalog 12 → 22 (+zendesk/intercom/freshdesk/pipedrive/greenhouse/mailchimp/google-analytics/stripe/confluence/airtable); covers support/CRM/recruiting/marketing/payments/docs domains; 5 tests, typecheck clean |
| 2026-06-25 | GTM3 done | MCP connector proxy provisioning runbook (operations/runbooks/) + `live` gate: catalog connectors are "coming soon" until their hosted proxy is provisioned — enable returns 409, UI badges + disables Connect. Prevents activating a tool that no-ops. 32 tests green; both typechecks clean |
| 2026-06-25 | GTM4 done | Operator "Connector Status" panel in internal dashboard — read-only view of the 22 managed connectors grouped by category with Live / Coming-soon badges + summary counts; reuses the `live` flag + GET /v1/mcp/catalog. Dashboard proxy + sidebar link. Typecheck clean (live preview env-gated) |
