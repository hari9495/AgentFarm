...
describe('SupportChatSession — inactivity timeout', () => {
    it('closes WebSocket after inactivity timeout fires', async () => {
        ...
        const p = (app.server.address() as { port: number }).port;

        const { ws, frames } = await connectWs(p);
        await waitFrames(frames, 1); // connected frame

        const closed = await new Promise<boolean>((resolve) => {
            ws.on('close', () => resolve(true));
            setTimeout(() => resolve(false), 600);
        });
        assert.equal(closed, true, 'WebSocket should close after inactivity timeout');

        await app.close();
    });

    it('sends error frame before closing on timeout', async () => {
        const app = Fastify({ logger: false });
        await registerSupportChatSessionRoutes(app, {
            getSession: () => SESSION,
            gatewayBaseUrl: `http://127.0.0.1:${mockGatewayPort}`,
        ...
        assert.ok(errFrame, 'expected error frame before close');
        assert.ok(errFrame.text?.toLowerCase().includes('inactiv') || errFrame.text?.toLowerCase().includes('timeout'));

        await app.close();
    });

    it('resets timeout on incoming message — socket stays open', async () => {
        const app = Fastify({ logger: false });
        await registerSupportChatSessionRoutes(app, {
            getSession: () => SESSION,
            gatewayBaseUrl: `http://127.0.0.1:${mockGatewayPort}`,
        ...
        // Send a message just before the 300ms would fire — resets the clock
        await new Promise((r) => setTimeout(r, 200));
        ws.send(JSON.stringify({ type: 'message', text: 'keep alive' }));

        // At 350ms total, only 150ms since the last message — should still be open
        await new Promise((r) => setTimeout(r, 150));
        assert.notEqual(ws.readyState, WebSocket.CLOSED, 'socket should still be open after timer reset');
