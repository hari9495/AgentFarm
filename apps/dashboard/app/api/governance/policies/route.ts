...
export async function GET(request: Request) {
    ...
    const upstream = `${getApiBaseUrl()}/v1/governance/policies?${searchParams.toString()}`;
    const response = await fetch(upstream, { headers: { Authorization: authHeader }, cache: 'no-store' });
    const body = await response.json().catch(() => ({ error: 'upstream_error', message: 'Unable to parse policy response.' }));
...
export async function POST(request: Request) {
    ...
    const response = await fetch(`${getApiBaseUrl()}/v1/governance/policies`, {
        method: 'POST',
