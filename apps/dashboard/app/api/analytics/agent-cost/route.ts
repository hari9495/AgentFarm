...
export async function GET(request: NextRequest) {
    ...
    const upstreamUrl =
        `${getApiBaseUrl()}/v1/analytics/agent-cost` +
    ...
    let res: Response;
    try {
        res = await fetch(upstreamUrl, { headers: { Authorization: authHeader }, cache: 'no-store' });
    } catch {
        return NextResponse.json({ error: 'upstream_error', message: 'Failed to reach analytics service.' }, { status: 502 });
    }

    const body = await res.json().catch(() => ({ error: 'upstream_error', message: 'Unable to parse response.' }));
