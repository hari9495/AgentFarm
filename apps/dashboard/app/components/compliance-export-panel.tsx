...
export default function ComplianceExportPanel() {
    ...
    if (error) return <div style={{ ...card, borderColor: '#2563EB', display: 'flex', gap: 8, alignItems: 'center', fontSize: 13, color: '#2563EB' }}><AlertCircle size={15} />{error}</div>;
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            {report.integrity && (
                <div style={{ ...card, display: 'flex', alignItems: 'center', gap: 10, borderColor: report.integrity.chainValid ? 'var(--accent)' : '#2563EB' }}>
                    {report.integrity.chainValid
                        ? <ShieldCheck size={18} color="var(--accent)" />
                        : <Ban size={18} color="#2563EB" />}
                    <div>
                        <div style={{ fontWeight: 700, fontSize: 13, color: report.integrity.chainValid ? 'var(--accent)' : '#2563EB' }}>
                        ...
                        {report.violations.map((v) => (
                            <div key={v.id} style={{ padding: '8px 10px', borderRadius: 8, border: '1px solid var(--line)' }}>
                                <div style={{ fontSize: 13, ...mono }}>
                                    <span style={{ color: '#2563EB', fontWeight: 700 }}>{v.effect}</span> {v.actionType}
