...
const statusConfig: Record<Status, { icon: typeof CheckCircle; label: string; color: string; bg: string }> = {
    ...
    planned: { icon: Circle, label: "Planned", color: "var(--op-muted)", bg: "rgba(174,174,178,0.08)" },
};
            ...
            <section className="af-tile af-tile-white text-center" style={{ paddingTop: 80, paddingBottom: 56 }}>
                <div className="af-container-narrow">
                    ...
                    <h1 className="font-semibold text-[var(--op-ink)]" style={{ fontSize: "clamp(2.4rem, 5vw, 3.6rem)", letterSpacing: "-0.03em", lineHeight: 1.07 }}>
                        ...
                        <span className="text-[var(--op-indigo)]">{changelogListingContent.hero.titleAccent}</span>
                    </h1>
                    <p className="mt-5 text-[17px] text-[var(--op-ink-soft)] max-w-lg mx-auto" style={{ lineHeight: 1.47 }}>
                    ...
                    <div className="flex flex-wrap gap-2 mb-8">
                        {filters.map((f) => (
                            <button
                                ...
                                style={{
                                    background: filter === f ? "var(--op-indigo)" : "#ffffff",
                                    color: filter === f ? "#ffffff" : "var(--op-muted)",
                                    border: filter === f ? "none" : "1px solid var(--op-line)",
                                }}
                            >
                                {f}
                            </button>
                                    ...
                                    <div className="flex flex-col sm:flex-row sm:items-start gap-4">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <div className="flex items-baseline gap-3 mb-2">
                                                {version && (
                                                    <span className="font-mono text-[13px] font-semibold text-[var(--op-indigo)]">{version}</span>
                                                )}
                                                <span className="text-[13px] text-[var(--op-muted)]">{entry.date}</span>
                                            </div>
                                            {slug ? (
                                                <Link href={`/changelog/${slug}`} className="block">
                                                    <h3 className="font-semibold text-[17px] text-[var(--op-ink)] mb-2 hover:text-[var(--op-indigo)] transition-colors" style={{ letterSpacing: "-0.018em" }}>
                                                        {entry.title}
                                                    </h3>
                                                </Link>
                                            ) : (
                                            ...
                                            <p className="text-[14px] text-[var(--op-muted)] mb-3" style={{ lineHeight: 1.5 }}>{summary}</p>
                                            <div className="flex flex-wrap gap-1.5">
                                                {entry.tags.map((tag) => (
                                                    <span
                                                        key={tag}
                                                        className="text-[11px] font-semibold px-2 py-0.5 rounded-full"
                                                        style={{ background: "rgba(37,99,235,0.06)", color: "var(--op-indigo)" }}
                                                    >
                                                        {tag}
                                                    </span>
                                                ))}
