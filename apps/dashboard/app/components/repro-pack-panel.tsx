...
function CopyButton({ text }: { text: string }) {
    ...
    return (
        <button
            ...
            style={{ display: 'inline-flex', alignItems: 'center', gap: 4, padding: '3px 8px', borderRadius: 9999, border: '1px solid #d2d2d7', background: '#fff', color: copied ? '#1a7a4a' : '#6e6e73', fontSize: 11, fontWeight: 600, cursor: 'pointer', transition: 'all 0.15s' }}
        >
...
function PackRow({ pack }: { pack: ReproPack }) {
    ...
    return (
        <div style={{ border: '1px solid #d2d2d7', borderRadius: 14, background: '#fff', overflow: 'hidden', boxShadow: '0 1px 3px rgba(0,0,0,0.05)' }}>
            <button
                type="button"
            ...
            >
                <div style={{ width: 34, height: 34, borderRadius: 9, background: 'rgba(0,102,204,0.08)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                    <Package size={15} color="#0066cc" />
                </div>
                <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 13, fontWeight: 600, color: '#1d1d1f', fontFamily: 'monospace' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
                    <span style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 11, color: '#6e6e73' }}>
            ...
            {expanded && (
                <div style={{ borderTop: '1px solid #f0f0f2', padding: '14px 16px', background: '#f5f5f7' }}>
                    <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 12 }}>
                        <CopyButton text={pack.id} />
                        {pack.taskId && (
                            <span style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '3px 10px', borderRadius: 9999, background: 'rgba(0,102,204,0.07)', border: '1px solid rgba(0,102,204,0.2)', fontSize: 11, fontWeight: 600, color: '#0066cc' }}>
...
export default function ReproPackPanel({ botId }: Props) {
    ...
    if (!botId) {
        return (
            <div style={{ padding: 24, textAlign: 'center', color: '#6e6e73', fontSize: 14 }}>
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            {/* Create form */}
            <div style={{ background: '#ffffff', border: '1px solid #d2d2d7', borderRadius: 18, padding: 20, boxShadow: '0 1px 3px rgba(0,0,0,0.05)' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
                    <div style={{ width: 32, height: 32, borderRadius: 9, background: 'rgba(0,102,204,0.08)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                        <Package size={15} color="#0066cc" />
                    </div>
                    <div>
                        <div style={{ fontSize: 14, fontWeight: 700, color: '#1d1d1f', letterSpacing: '-0.01em' }}>Create Repro Pack</div>
                        <div style={{ fontSize: 12, color: '#6e6e73' }}>Bundle a task's context into a reproducible debug snapshot</div>
                    </div>
                </div>
                ...
                <form onSubmit={createPack} style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                        <div>
                            <label style={{ fontSize: 11, fontWeight: 600, color: '#424245', display: 'block', marginBottom: 4 }}>
                                Task ID <span style={{ color: '#aeaeb2', fontWeight: 400 }}>(optional)</span>
                            </label>
                            <input value={taskId} onChange={e => setTaskId(e.target.value)} style={inputStyle} placeholder="task_..." />
                        </div>
                        <div>
                            <label style={{ fontSize: 11, fontWeight: 600, color: '#424245', display: 'block', marginBottom: 4 }}>Description</label>
                    ...
                    <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                        ...
                        <button type="button" onClick={loadPacks} disabled={loading} style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '7px 14px', borderRadius: 9999, border: '1px solid #d2d2d7', background: '#fff', color: '#424245', fontSize: 13, fontWeight: 500, cursor: loading ? 'not-allowed' : 'pointer' }}>
            ...
            {/* Pack list */}
            {error && (
                <div style={{ padding: '10px 14px', borderRadius: 10, background: 'rgba(196,22,28,0.07)', border: '1px solid rgba(196,22,28,0.2)', color: '#c4161c', fontSize: 13 }}>
                    {error}
                </div>
            )}

            {loaded && packs.length === 0 && !error && (
                <div style={{ padding: '20px', textAlign: 'center', color: '#6e6e73', fontSize: 13, border: '1px dashed #d2d2d7', borderRadius: 14 }}>
            ...
            {packs.length > 0 && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                    <div style={{ fontSize: 11, fontWeight: 700, color: '#6e6e73', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 4 }}>
