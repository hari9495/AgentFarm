// 21st.dev component — badtzx0/animated-card-chart (hover-morphing bar chart card)
'use client';
...
export function AnimatedCard({ className, ...props }: CardProps) {
    return (
        <div
            role="region"
            className={cn(
                'group/animated-card relative w-full overflow-hidden rounded-xl border border-zinc-200 bg-white shadow-sm dark:border-zinc-800 dark:bg-[#0F1418]',
                className,
            )}
            {...props}
        />
...
export function CardBody({ className, ...props }: CardProps) {
    return <div role="group" className={cn('flex flex-col space-y-1.5 border-t border-zinc-200 p-4 dark:border-zinc-800', className)} {...props} />;
}
...
interface Visual3Props { mainColor?: string; secondaryColor?: string; gridColor?: string }

export function Visual3({ mainColor = '#37A0A0', secondaryColor = '#D2A24E', gridColor = '#80808015' }: Visual3Props) {
...
const GridLayer: React.FC<{ color: string }> = ({ color }) => (
    <div style={{ '--grid-color': color } as React.CSSProperties}
...
const EllipseGradient: React.FC<{ color: string }> = ({ color }) => (
    <div className="absolute inset-0 z-[5] flex h-full w-full items-center justify-center">
        <svg width="356" height="196" viewBox="0 0 356 180" fill="none" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="none" className="w-full">
            ...
            <defs>
                <radialGradient id="paint0_radial_12_207" cx="0" cy="0" r="1" gradientUnits="userSpaceOnUse" gradientTransform="translate(178 98) rotate(90) scale(98 178)">
                    ...
                    <stop offset="0.34" stopColor={color} stopOpacity="0.16" />
...
const Layer2: React.FC<{ color: string }> = ({ color }) => (
    <div className="group relative h-full w-full" style={{ '--color': color } as React.CSSProperties}>
        <div className="absolute inset-0 z-[7] flex w-full translate-y-full items-start justify-center bg-transparent p-4 transition-transform duration-500 group-hover/animated-card:translate-y-0">
            <div className="rounded-md border border-zinc-200 bg-white/25 p-1.5 opacity-0 backdrop-blur-sm transition-opacity duration-500 group-hover/animated-card:opacity-100 dark:border-zinc-800 dark:bg-black/25">
                <div className="flex items-center gap-2">
                    <div className="h-2 w-2 shrink-0 rounded-full bg-[var(--color)]" />
                    <p className="text-xs text-black dark:text-white">Throughput last 24h</p>
                </div>
                <p className="text-xs text-neutral-500 dark:text-neutral-400">Tasks executed per hour.</p>
            </div>
        </div>
    </div>
);
...
const Layer3: React.FC<{ color: string }> = ({ color }) => (
    <div className="absolute inset-0 z-[6] flex translate-y-full items-center justify-center opacity-0 transition-all duration-500 group-hover/animated-card:translate-y-0 group-hover/animated-card:opacity-100">
        <svg width="356" height="180" viewBox="0 0 356 180" fill="none" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="none" className="w-full h-full">
...
const Layer4: React.FC<LayerProps> = ({ color, secondaryColor, hovered }) => {
    ...
    return (
        <div className="absolute inset-0 z-[8] flex h-[180px] w-full items-center justify-center text-neutral-800/10 transition-transform duration-500 group-hover/animated-card:scale-150 dark:text-white/15">
            <svg width="356" height="180" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="none" className="w-full">
                {rectsData.map((rect, index) => (
                    <rect key={index} width={rect.width} height={hovered ? rect.hoverHeight : rect.height} x={rect.x} y={hovered ? rect.hoverY : rect.y}
                        fill={hovered ? rect.hoverFill : rect.fill} rx="2" ry="2" className="transition-all duration-500" />
                ))}
            </svg>
        </div>
    );
