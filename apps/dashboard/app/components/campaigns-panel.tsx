...
const STATUS_STYLE: Record<CampaignStatus, { bg: string; color: string; icon: React.ElementType }> = {
    ...
    pending_approval: { bg: 'var(--warn-bg)', color: 'var(--warn)', icon: Clock },
    approved:         { bg: 'var(--ok-bg)', color: 'var(--ok)', icon: CheckCircle2 },
    active:           { bg: 'var(--info-bg)', color: 'var(--info)', icon: Play },
    paused:           { bg: 'var(--warn-bg)', color: 'var(--warn)', icon: Pause },
    completed:        { bg: 'var(--ok-bg)', color: 'var(--ok)', icon: CheckCircle2 },
    rejected:         { bg: 'var(--danger-bg)', color: 'var(--danger)', icon: Clock },
};
                            ...
                            {campaigns.map((c, i) => {
                                ...
                                return (
                                    <tr key={c.id} style={{ borderTop: i === 0 ? 'none' : '1px solid #f1f5f9', background: i % 2 === 1 ? 'var(--bg)' : 'var(--card)' }}>
                                        ...
                                        <td style={td}><span style={{ fontSize: 11, padding: '2px 7px', borderRadius: 4, background: 'var(--bg)', color: 'var(--ink-muted)' }}>{CHANNEL_LABEL[c.channel]}</span></td>
                                        <td style={{ ...td, color: 'var(--ink-muted)' }}>${c.budgetUsd.toLocaleString()}</td>
                                        ...
                                        <td style={{ ...td, color: 'var(--ink-muted)' }}>{c.conversions?.toLocaleString() ?? '—'}</td>
