...
function CopyBtn({ text, label = "Copy" }: { text: string; label?: string }) {
    ...
    return (
        <button onClick={() => { void navigator.clipboard.writeText(text); setCopied(true); setTimeout(() => setCopied(false), 2000); }}
            className={`flex items-center gap-1.5 text-xs font-semibold rounded-lg px-3 py-1.5 border transition-colors ${copied ? "border-[color:color-mix(in_srgb,var(--ok)_40%,transparent)] dark:border-[color:color-mix(in_srgb,var(--ok)_40%,transparent)] bg-[color-mix(in_srgb,var(--ok)_10%,transparent)] dark:bg-[color-mix(in_srgb,var(--ok)_22%,transparent)]/30 text-[color:var(--ok)] dark:text-[color:var(--ok)]" : "border-[color:var(--line)] dark:border-[color:var(--line)] bg-[var(--card)] dark:bg-[var(--card)] text-[color:var(--ink-soft)] dark:text-[color:var(--ink-muted)] hover:border-[color:var(--line)]"}`}>
    ...
    return (
        <div className="min-h-svh bg-[var(--bg-deep)]">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-10 py-6 space-y-6">
                ...
                <section className="relative overflow-hidden rounded-xl border border-[color:var(--line)] bg-[var(--card)]">
                    ...
                    <div className="relative px-6 sm:px-8 py-6 sm:py-8">
                        ...
                        <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-5">
                            <div>
                                <h1 className="text-2xl sm:text-[28px] font-semibold text-[color:var(--ink)] tracking-tight leading-tight">Inbound Webhooks</h1>
                ...
                {/* ── New secret reveal ─────────────────────────────── */}
                {newSource && (
                    <div className="rounded-xl border border-[color:color-mix(in_srgb,var(--ok)_40%,transparent)] dark:border-[color:color-mix(in_srgb,var(--ok)_40%,transparent)]/50 bg-[color-mix(in_srgb,var(--ok)_10%,transparent)] dark:bg-[color-mix(in_srgb,var(--ok)_22%,transparent)]/20 p-5">
                            ...
                            {[{ label: "Webhook URL", value: newSource.url }, { label: "Signing Secret", value: newSource.secret }].map(({ label, value }) => (
                                <div key={label}>
                                    ...
                                    <div className="flex items-center gap-2">
                                        <code className="flex-1 font-mono text-xs bg-[var(--card)] dark:bg-[var(--card)] border border-[color:color-mix(in_srgb,var(--ok)_40%,transparent)] dark:border-[color:color-mix(in_srgb,var(--ok)_40%,transparent)]/50 rounded-lg px-3 py-2 overflow-hidden text-ellipsis whitespace-nowrap text-[color:var(--ink)] dark:text-[color:var(--ink)]">
                                            {value}
                                        </code>
                                        <CopyBtn text={value} />
                                    </div>
                ...
                {/* ── Tab bar ──────────────────────────────────────────── */}
                <div className="flex gap-1 p-1 rounded-xl bg-[var(--bg-deep)] dark:bg-[var(--card)]/60 w-fit">
                    {(["sources", "events"] as const).map(tab => (
                        <button key={tab} onClick={() => setActiveTab(tab)}
                            className={`px-5 py-2.5 rounded-lg text-sm font-semibold transition-all ${activeTab === tab ? "bg-[var(--card)] dark:bg-[var(--card)] text-[color:var(--ink)] dark:text-[color:var(--ink)] shadow-sm" : "text-[color:var(--ink-muted)] dark:text-[color:var(--ink-muted)] hover:text-[color:var(--ink-soft)] dark:hover:text-[color:var(--ink)]"}`}>
                ...
                {activeTab === "sources" && (
                    <div className="space-y-4">
                        ...
                        {showForm && (
                            <div className="rounded-xl border-2 border-[color:color-mix(in_srgb,var(--accent)_40%,transparent)] dark:border-[color:color-mix(in_srgb,var(--accent)_40%,transparent)] bg-[var(--card)] dark:bg-[var(--card)] overflow-hidden shadow-sm">
                                ...
                                <form onSubmit={createSource} className="p-6 space-y-4">
                                    ...
                                    {/* How it works */}
                                    <div className="rounded-lg bg-[var(--bg-deep)] dark:bg-[var(--card)]/50 border border-[color:var(--line)] dark:border-[color:var(--line)] px-4 py-3 text-sm text-[color:var(--ink-soft)] dark:text-[color:var(--ink-muted)]">
                                    ...
                                    {addError && <div className="flex items-center gap-2 text-sm text-[color:var(--danger)] dark:text-[color:var(--danger)] bg-[color-mix(in_srgb,var(--danger)_10%,transparent)] dark:bg-[color-mix(in_srgb,var(--danger)_22%,transparent)]/30 border border-[color:color-mix(in_srgb,var(--danger)_40%,transparent)] dark:border-[color:color-mix(in_srgb,var(--danger)_40%,transparent)]/40 rounded-lg px-4 py-2.5"><AlertCircle className="w-4 h-4 shrink-0" />{addError}</div>}
                        ...
                        {/* Source list */}
                        {loading ? (
                            <div className="space-y-3">{[1,2].map(i => <div key={i} className="h-24 rounded-xl bg-[var(--line)] dark:bg-[var(--card)] animate-pulse" />)}</div>
                            ...
                            <div className="space-y-3">
                                {sources.map(source => (
                                    <div key={source.id} className="rounded-xl border border-[color:var(--line)] dark:border-[color:var(--line)] bg-[var(--card)] dark:bg-[var(--card)] p-5 shadow-sm">
                                        <div className="flex items-start gap-3">
                                            <div className="w-9 h-9 rounded-lg bg-[color-mix(in_srgb,var(--accent)_10%,transparent)] dark:bg-[color-mix(in_srgb,var(--accent)_22%,transparent)]/40 flex items-center justify-center shrink-0">
                                            ...
                                            <div className="flex-1 min-w-0">
                                                ...
                                                <div className="flex items-center gap-2 mb-2">
                                                    <code className="flex-1 font-mono text-xs bg-[var(--bg-deep)] dark:bg-[var(--card)] border border-[color:var(--line)] dark:border-[color:var(--line)] rounded-lg px-3 py-1.5 overflow-hidden text-ellipsis whitespace-nowrap text-[color:var(--accent)] dark:text-[color:var(--accent)]">
                                                        {source.inboundUrl}
                                                    </code>
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="flex gap-3 items-center flex-wrap">
                            <select value={filterSource} onChange={e => setFilterSource(e.target.value)}
                                className="border border-[color:var(--line)] dark:border-[color:var(--line)] bg-[var(--card)] dark:bg-[var(--card)] text-[color:var(--ink)] dark:text-[color:var(--ink)] rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[color:color-mix(in_srgb,var(--accent)_40%,transparent)]">
                            ...
                            <button onClick={() => void loadEvents()} disabled={loading}
                                className="flex items-center gap-1.5 text-sm font-semibold border border-[color:var(--line)] dark:border-[color:var(--line)] text-[color:var(--ink-soft)] dark:text-[color:var(--ink-muted)] rounded-lg px-3 py-2 hover:bg-[var(--bg-deep)] dark:hover:bg-[var(--card)] disabled:opacity-50 transition-colors">
                                <RefreshCw className={`w-3.5 h-3.5 ${loading ? "animate-spin" : ""}`} /> Refresh
                            </button>
                        </div>

                        {loading ? (
                            <div className="space-y-2">{[1,2,3].map(i => <div key={i} className="h-16 rounded-lg bg-[var(--line)] dark:bg-[var(--card)] animate-pulse" />)}</div>
                        ) : events.length === 0 ? (
                            <div className="rounded-xl border border-dashed border-[color:var(--line)] dark:border-[color:var(--line)] p-10 text-center">
                            ...
                            <div className="rounded-xl border border-[color:var(--line)] dark:border-[color:var(--line)] bg-[var(--card)] dark:bg-[var(--card)] overflow-hidden shadow-sm">
