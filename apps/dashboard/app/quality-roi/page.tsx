...
function ScoreBar({ value, max }: { value: number | null; max: number }) {
    ...
    return (
        <div style={{ width: '100%', background: '#f3f4f6', borderRadius: 4, height: 8, overflow: 'hidden' }}>
    ...
    return (
        <div style={{ minHeight: '100vh', background: '#f9fafb' }}>
            ...
            <main style={{ maxWidth: 1100, margin: '0 auto', padding: '1.5rem', display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                    ...
                    <div style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap' }}>
                        {[
                            ...
                            <div key={label} style={{
                                ...
                                <p style={{ margin: '0 0 0.2rem', fontSize: '0.72rem', color: '#6b7280', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em' }}>{label}</p>
                                <p style={{ margin: 0, fontSize: '1.35rem', fontWeight: 700, color: highlight ?? '#111827' }}>{value}</p>
                                <p style={{ margin: '0.15rem 0 0', fontSize: '0.72rem', color: '#9ca3af' }}>{sub}</p>
                            </div>
                        ))}
                    </div>
                )}
                ...
                {/* ROI explanation */}
                <div style={{ padding: '0.75rem 1rem', background: '#eff6ff', border: '1px solid #bfdbfe', borderRadius: 8, fontSize: '0.82rem', color: '#1e40af' }}>
                ...
                <section style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 12, padding: '1.25rem 1.5rem', overflowX: 'auto' }}>
                    <h2 style={{ fontSize: '1rem', fontWeight: 700, color: '#111827', margin: '0 0 1rem' }}>Weekly ROI Breakdown</h2>
                        ...
                        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.82rem' }}>
                            <thead>
                                <tr style={{ borderBottom: '2px solid #e5e7eb', textAlign: 'left' }}>
                                    ...
                                    <th style={{ padding: '0.4rem 0.6rem', fontWeight: 600, color: '#374151', textAlign: 'right' }}>Cost / Success</th>
                                    <th style={{ padding: '0.4rem 0.6rem', fontWeight: 600, color: '#374151' }}>ROI Score</th>
                                    <th style={{ padding: '0.4rem 0.6rem', fontWeight: 600, color: '#374151', minWidth: 100 }}>ROI Bar</th>
                                </tr>
                            </thead>
                            <tbody>
                                {rows.map((r) => (
                                    <tr key={r.week} style={{ borderBottom: '1px solid #f3f4f6' }}>
                                        <td style={{ padding: '0.5rem 0.6rem', color: '#374151', fontFamily: 'monospace' }}>{r.week}</td>
                                        <td style={{ padding: '0.5rem 0.6rem', textAlign: 'right' }}>{r.tasks.toLocaleString()}</td>
                                        <td style={{ padding: '0.5rem 0.6rem', textAlign: 'right', color: '#16a34a' }}>{r.successes.toLocaleString()}</td>
                                        ...
                                        <td style={{ padding: '0.5rem 0.6rem', textAlign: 'right', color: '#6b7280' }}>${r.cost.toFixed(3)}</td>
                                        ...
                                        <td style={{ padding: '0.5rem 0.6rem' }}>
                                            ...
                                            <span style={{ fontSize: '0.65rem', color: '#9ca3af' }}>{roiLabel(r.roiScore)}</span>
                                        </td>
                                        <td style={{ padding: '0.5rem 0.6rem' }}>
                ...
                {!isLoading && rows.length > 1 && (
                    <section style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 12, padding: '1.25rem 1.5rem' }}>
                        <h2 style={{ fontSize: '1rem', fontWeight: 700, color: '#111827', margin: '0 0 0.75rem' }}>ROI Score Trend</h2>
                            ...
                            {rows.map((r) => {
                                ...
                                return (
                                    <div key={r.week} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
                                        {r.roiScore !== null && (
                                            <span style={{ fontSize: '0.6rem', color: '#6b7280' }}>{r.roiScore.toFixed(0)}</span>
                                        )}
                                        <div
                                        ...
                                        />
                                        <span style={{ fontSize: '0.6rem', color: '#9ca3af' }}>{r.week.slice(5)}</span>
                                    </div>
                                );
                            })}
                        </div>
