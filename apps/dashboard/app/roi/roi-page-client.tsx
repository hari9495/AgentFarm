...
function SparkBars({ points, valueKey }: { points: WeekPoint[]; valueKey: 'taskCount' | 'totalCostUsd' }) {
    if (points.length === 0) return <span style={{ color: '#a8a29e', fontSize: '0.78rem' }}>No data</span>;
            ...
            {points.map((p, i) => {
                ...
                return (
                    <div
                        ...
                        style={{
                            flex: 1, height: h,
                            background: isLast ? '#0052cc' : '#bfdbfe',
...
function KpiCard({
    ...
    return (
        <div className="card" style={{ padding: '1rem 1.15rem' }}>
            <div style={{ fontSize: '0.72rem', fontWeight: 700, color: '#78716c', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: '0.4rem' }}>
                {label}
            </div>
            <div style={{ fontSize: '1.6rem', fontWeight: 800, color: accent ?? '#1d1d1f', lineHeight: 1.1 }}>
                {value}
            </div>
            {sub && <div style={{ fontSize: '0.75rem', color: '#78716c', marginTop: '0.25rem' }}>{sub}</div>}
            {deltaObj && (
                <div style={{ fontSize: '0.75rem', marginTop: '0.25rem', fontWeight: 600, color: deltaObj.up ? '#1a7a4a' : '#c4161c' }}>
    ...
    return (
        <div style={{ display: 'grid', gap: '1.5rem' }}>
            ...
            <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center', flexWrap: 'wrap' }}>
                <span style={{ fontSize: '0.8rem', fontWeight: 600, color: '#57534e' }}>Period:</span>
                {[7, 30, 60, 90].map((d) => (
                    <button
                        key={d}
                        type="button"
                ...
                {loading && <span style={{ fontSize: '0.78rem', color: '#78716c' }}>Loading…</span>}
            </div>

            {error && <p className="message-inline" style={{ color: '#c4161c' }}>{error}</p>}
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))', gap: '0.75rem' }}>
                ...
                <KpiCard
                    ...
                    accent={netValue >= 0 ? '#166534' : '#c4161c'}
                />
                <KpiCard
                    label="ROI Multiple"
                    value={roiMultiple !== null ? `${roiMultiple.toFixed(1)}×` : '—'}
                    sub={roiMultiple !== null ? `${Math.round((roiMultiple - 1) * 100)}% return` : 'no cost yet'}
                    accent={roiMultiple !== null && roiMultiple >= 1 ? '#166534' : '#57534e'}
                />
            </div>

            {/* ── Assumptions panel ── */}
            <div className="card" style={{ padding: '1rem 1.15rem' }}>
                <h3 style={{ margin: '0 0 0.75rem', fontSize: '0.88rem', fontWeight: 700, color: '#374151' }}>
                    Adjust assumptions
                </h3>
                <div style={{ display: 'flex', gap: '2rem', flexWrap: 'wrap', alignItems: 'flex-end' }}>
                    <label style={{ display: 'grid', gap: '0.3rem', fontSize: '0.8rem', fontWeight: 600, color: '#57534e' }}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                            <input
                                ...
                                style={{ width: 140 }}
                            />
                            <span style={{ fontSize: '0.88rem', fontWeight: 700, color: '#1d1d1f', minWidth: 40 }}>
                                {minsPerTask} min
                            </span>
                        </div>
                    </label>
            ...
            {perf && perf.weeklyTrend.length > 0 && (
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem' }}>
                    <div className="card" style={{ padding: '1rem 1.15rem' }}>
                        <div style={{ fontSize: '0.78rem', fontWeight: 700, color: '#57534e', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '0.6rem' }}>
                            Weekly task volume
                        </div>
                        <SparkBars points={perf.weeklyTrend} valueKey="taskCount" />
                        <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.68rem', color: '#a8a29e', marginTop: '0.3rem' }}>
                ...
                <div className="card" style={{ padding: '1rem 1.15rem' }}>
                    ...
                    <div style={{ display: 'grid', gap: '0.5rem' }}>
                        {topAgents.map((a) => {
                            ...
                            return (
                                <div key={a.botId} style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', padding: '0.6rem 0.75rem', background: '#f9fafb', borderRadius: '0.4rem', flexWrap: 'wrap' }}>
                                    <span style={{ padding: '0.15rem 0.5rem', borderRadius: 9999, background: 'rgba(0,82,204,0.08)', color: '#0052cc', fontSize: '0.73rem', fontWeight: 700, textTransform: 'uppercase', flexShrink: 0 }}>
                                        {a.botRole ?? a.botId.slice(0, 10)}
                                    </span>
                                    <div style={{ display: 'flex', gap: '1.25rem', fontSize: '0.8rem', color: '#44403c', flexWrap: 'wrap' }}>
                                        ...
                                        {agentRoi !== null && (
                                            <span style={{ color: agentRoi >= 1 ? '#1a7a4a' : '#b45309' }}>
            ...
            {perf?.avgQualityScore !== null && perf?.avgQualityScore !== undefined && (
                <div className="card" style={{ padding: '0.85rem 1.15rem', display: 'flex', alignItems: 'center', gap: '1rem' }}>
                    <div>
                        <div style={{ fontSize: '0.72rem', fontWeight: 700, color: '#78716c', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Avg quality score</div>
                        <div style={{ fontSize: '1.4rem', fontWeight: 800, color: '#1d1d1f' }}>
                            {(perf.avgQualityScore * 100).toFixed(0)}
                            <span style={{ fontSize: '0.85rem', fontWeight: 500, color: '#78716c' }}>/100</span>
                        </div>
                    </div>
                    <div style={{ flex: 1, height: 8, background: '#f3f4f6', borderRadius: 9999, overflow: 'hidden' }}>
                        <div style={{ height: '100%', width: `${(perf.avgQualityScore * 100).toFixed(0)}%`, background: perf.avgQualityScore >= 0.7 ? '#1a7a4a' : perf.avgQualityScore >= 0.5 ? '#b45309' : '#c4161c', borderRadius: 9999, transition: 'width 0.4s' }} />
                    </div>
                    <div style={{ fontSize: '0.75rem', color: '#57534e', flexShrink: 0 }}>
            ...
            {!loading && perf?.taskCount === 0 && (
                <div className="card" style={{ padding: '3rem', textAlign: 'center' }}>
                    ...
                    <p style={{ margin: '0.3rem 0 0', fontSize: '0.82rem', color: '#78716c' }}>
