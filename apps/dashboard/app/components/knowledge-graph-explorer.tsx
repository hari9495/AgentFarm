...
const KIND_BADGE: Record<GraphSymbol['kind'], { bg: string; color: string; border: string }> = {
    function:  { bg: 'rgba(37, 99, 235,0.08)',   color: 'var(--accent)', border: 'rgba(37, 99, 235,0.2)'   },
                            ...
                            <div style={{ maxHeight: 480, overflowY: 'auto' }}>
                                {displaySymbols.slice(0, 100).map(sym => (
                                    <button key={`${sym.file_path}:${sym.name}`} onClick={() => void selectSymbol(sym)}
                                        style={{ width: '100%', display: 'flex', alignItems: 'flex-start', gap: 10, padding: '10px 14px', textAlign: 'left', border: 'none', borderBottom: '1px solid #f5f5f7', background: selected?.name === sym.name ? 'rgba(37, 99, 235,0.05)' : 'transparent', cursor: 'pointer', transition: 'background 0.1s' }}>
