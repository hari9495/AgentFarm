...
- Notes: Ten LLM providers supported (openai, azure_openai, github_models, anthropic, google, xai, mistral, together, agentfarm, auto). Auto mode iterates a per-profile priority list with health-score reordering; heuristic fallback fires if all providers fail. Model profiles: quality_first, speed_first, cost_balanced, custom. Dashboard LLM Config panel exposes all providers plus profile presets. See ADR-007.
