      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="max-w-3xl mx-auto">
          <div className="text-center mb-10">
            <span className="chip chip-accent">Live Demo</span>
            <h2 className="mt-4 text-3xl sm:text-4xl font-bold text-[var(--ink)] font-semibold tracking-[-0.03em]">
          ...
          {/* Terminal window */}
          <div className="rounded-2xl overflow-hidden border border-[var(--hairline)] shadow-2xl">
            {/* Title bar */}
            <div className="bg-[var(--surface-card)] px-5 py-3 flex items-center gap-3 border-b border-[var(--hairline)]">
            ...
            {/* Terminal body */}
            <div className="bg-[var(--surface)] px-6 py-5 font-mono text-sm min-h-[280px] relative">
              {!playing ? (
                <div className="absolute inset-0 flex items-center justify-center">
                  <button
                    onClick={handlePlay}
                    className="flex items-center gap-3 bg-[var(--accent-blue)] hover:bg-[#8dd7ff] text-[#07080a] font-semibold px-6 py-3 rounded-full transition-colors cursor-pointer"
                  >
                    <Play className="w-4 h-4 fill-[#07080a]" /> Play Demo
                  </button>
                </div>
              ) : (
                <div key={key} className="space-y-1.5">
                  ...
                  <motion.button
                    ...
                    transition={{ delay: 7 }}
                    onClick={handlePlay}
                    className="mt-4 text-xs text-[var(--ash)] hover:text-[var(--mute)] cursor-pointer transition-colors"
                  >
                    ↺ Replay
                  </motion.button>
                </div>
          ...
          <div className="mt-6 grid grid-cols-3 gap-4">
            {[
              ...
              <div key={s.label} className="text-center bg-[var(--surface-card)] rounded-xl py-4 border border-[var(--hairline)]">
