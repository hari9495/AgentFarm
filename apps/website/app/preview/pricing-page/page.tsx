...
export const metadata = { title: 'Pricing page preview' };
...
const plans: PricingPlan[] = PLAN_ORDER.map((tier) => {
  const tierBots = marketplaceBots.filter((b) => b.plan === tier);
  const liveTierBots = tierBots.filter((b) => b.available);
  const livePrices = liveTierBots.map((b) => b.priceMonthly);
  const minPrice = livePrices.length > 0 ? Math.min(...livePrices) : null;
  const rolePreview = tierBots.slice(0, 3).map((b) => b.name.replace(/^AI\s+/, '')).join(', ');
  const deptCount = new Set(tierBots.map((b) => b.department)).size;
  return {
    name: tier,
    ...
    features: [
      ...
      rolePreview ? `${pricingPageContent.planFeatures.topRolesPrefix} ${rolePreview}` : pricingPageContent.planFeatures.fallbackRoleSummary,
    ...
    <div>
      ...
      <section className="op-light relative overflow-hidden">
        ...
        <div className="op-wrap-narrow relative text-center" style={{ paddingTop: 88, paddingBottom: 56 }}>
          <p className="op-eyebrow">{pricingPageContent.hero.eyebrow}</p>
          <h1 className="font-[family-name:var(--font-display)] font-extrabold mt-4" style={{ fontSize: 'clamp(2.5rem, 5vw, 3.8rem)', letterSpacing: '-0.03em', lineHeight: 1.05, color: 'var(--op-ink)' }}>{pricingPageContent.hero.title}</h1>
          <p className="mt-5 mx-auto max-w-xl text-[1.075rem] leading-relaxed" style={{ color: 'var(--op-muted)' }}>{pricingPageContent.hero.description}</p>
          <p className="mt-3 text-[13px]" style={{ fontFamily: 'var(--font-mono)', color: 'var(--op-muted)' }}>{pricingPageContent.hero.footnoteTemplate.replace('{count}', String(availableMarketplaceBots.length))}</p>
        </div>
      </section>
