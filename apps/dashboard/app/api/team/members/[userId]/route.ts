...
export async function PATCH(
    ...
    const response = await fetch(
        `${getApiBaseUrl()}/v1/team/members/${encodeURIComponent(userId)}/role`,
        {
            method: 'PATCH',
    ...
    const body = await response.json().catch(() => ({ error: 'upstream_error', message: 'Unable to parse role update response.' }));
