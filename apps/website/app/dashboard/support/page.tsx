...
const STATUS_BADGE: Record<IssueStatus, string> = {
    ...
    fixing: "bg-violet-50 dark:bg-violet-950/30 text-violet-700 dark:text-violet-400",
            ...
            <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-10 py-6 space-y-6">
                ...
                <section className="relative overflow-hidden rounded-2xl bg-slate-950">
                    ...
                    <div className="relative px-6 sm:px-8 py-6 sm:py-8 flex flex-col sm:flex-row sm:items-end sm:justify-between gap-5">
                        <div>
                            <div className="flex items-center gap-2 rounded-xl bg-sky-500/10 border border-sky-500/20 px-3 py-1.5 text-xs font-bold uppercase tracking-widest text-sky-300 w-fit mb-4">
                                <PremiumIcon icon={LifeBuoy} tone="sky" containerClassName="w-4 h-4 rounded bg-sky-400/20 text-sky-300" iconClassName="w-2.5 h-2.5" />
                                Support
                            </div>
                            <h1 className="text-3xl sm:text-4xl font-extrabold text-white tracking-tight leading-tight">Support &amp; issues</h1>
                ...
                {showForm && (
                    <form onSubmit={handleSubmit} className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-5 space-y-4">
                        ...
                        <div className="flex items-center gap-3">
                            <button type="submit" disabled={submitting} className="rounded-lg bg-slate-900 dark:bg-white text-white dark:text-slate-900 text-sm font-bold px-4 py-2 disabled:opacity-60">
                                        ...
                                        >
                                            ...
                                            <div className="min-w-0 flex-1">
                                                <div className="flex items-center gap-2 flex-wrap">
                                                    ...
                                                    {issue.prUrl && (
                                                        <a href={issue.prUrl} target="_blank" rel="noreferrer" onClick={(e) => e.stopPropagation()} className="text-xs font-semibold text-sky-600 dark:text-sky-400 hover:underline">
                                                            View fix PR →
                                                        </a>
                                                    )}
                                                </div>
