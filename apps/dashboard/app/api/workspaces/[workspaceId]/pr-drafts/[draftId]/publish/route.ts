...
export async function POST(request: Request, { params }: RouteParams) {
    ...
    try {
        const response = await fetch(
            `${getApiBaseUrl()}/v1/workspaces/${encodeURIComponent(workspaceId)}/pull-requests/${encodeURIComponent(draftId)}/publish`,
            {
                method: 'POST',
                headers: {
        ...
        const data = await response.json().catch(() => ({
            ...
            message: 'Unable to parse publish response.',
        }));

        return NextResponse.json(data, { status: response.status });
    } catch {
        return NextResponse.json(
            { error: 'upstream_unavailable', message: 'PR publish upstream is unavailable.' },
            { status: 502 },
        );
    }
}
